...
exports.createStructure = async (req, res) => {
    console.log('=== CREATE STRUCTURE CALLED ===');
    console.log('Body:', JSON.stringify(req.body, null, 2));
    
    try {
        // Récupérer les données
        const { code_structure, nom_structure, description, region_id } = req.body;
        
        // Valider les champs requis
        if (!code_structure) {
            return res.status(400).json({ error: 'code_structure is required' });
        }
        if (!nom_structure) {
            return res.status(400).json({ error: 'nom_structure is required' });
        }
        if (!region_id) {
        ...
        const structure = new Structure({
            code_structure: code_structure.toUpperCase(),
        ...
        console.log('Structure created:', structure._id);
        res.status(201).json({
            success: true,
...
exports.getAllStructures = async (req, res) => {
    try {
        const structures = await Structure.find({ is_active: true })
            .populate('region', 'nom_region code_region');
...
exports.updateStructure = async (req, res) => {
    try {
        ...
        if (req.body.nom_structure) structure.nom_structure = req.body.nom_structure;
        if (req.body.description !== undefined) structure.description = req.body.description;
        if (req.body.code_structure) structure.code_structure = req.body.code_structure.toUpperCase();
